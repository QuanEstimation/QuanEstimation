...
from Common.common import suN_generator
#===============================================================================
#Subclass: metrology
#===============================================================================
class CramerRao():
    ...
    def __init__(self, precision=1e-10):
        ...
        self.suN_generator = None

    def CFIM(self, rho, drho, M):
        """
    ...
    def SLD(self, rho, drho, rep='original'):
        """
        ...
        if np.abs(1-purity) < self.precision:
            ...
            for para_i in range(0, para_num):
                ...
                for fi in range (0, dim):
                    for fj in range (0, dim):
                        if np.abs(val[fi]+val[fj])> self.precision:
    ...
    def RLD(self, rho, drho, rep='original'):
        """
        ...
        for para_i in range(0, para_num):
            ...
            for fi in range (0, dim):
                for fj in range (0, dim):
                    if np.abs(val[fi])> self.precision:
    ...
    def LLD(self, rho, drho, rep='original'):
        """
        ...
        for para_i in range(0, para_num):
            ...
            for fi in range (0, dim):
                for fj in range (0, dim):
                    if np.abs(val[fj])> self.precision:
    ...
    def QFIM(self, rho, drho, rho_type='density_matrix', dtype='SLD', exportLD=False):
        """
        ...
        if rho_type == 'Bloch_vector':
            r = rho
            dr = drho
            self.suN_generator = suN_generator(dim)
            dim = int(np.sqrt(len(r)+1))
            rho = (np.qeye(dim)+np.sqrt(dim*(dim-1)/2)*np.inner(r,self.suN_generator))/dim
            drho = [[] for i in range(0, para_num)]
            for para in range(para_num):
                drho[para] = np.sqrt((dim-1)/(2*dim))*np.inner(dr[para],suN_generator)
        #===========================================================   
        if dtype=='SLD':
            LD_tp = self.SLD(rho, drho)
            for para_i in range(0, para_num):
                for para_j in range(para_i, para_num):
