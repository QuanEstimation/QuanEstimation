...
for ei in range(episodes):
    # state optimization
    DE_paras = {"popsize":10, "psi0":psi_opt, "max_episode":50, "c":1.0, "cr":0.5, "seed":1234}
    state = StateOpt(savefile=True, method="DE", **DE_paras)
    state.dynamics(tspan, H0, dH, Hc=Hc, ctrl=ctrl_opt, decay=decay, )
    state.QFIM()
    #  load f and rename
    f_load = open('f.csv', 'r')
    ...
    DE_paras = {"popsize":10, "ctrl0":ctrl0, "max_episode":50, "c":1.0, "cr":0.5, "seed":1234}
    control = ControlOpt(tspan, rho_opt, H0, dH, Hc, decay=decay, ctrl_bound=[-0.2, 0.2], savefile=True, \
                         method="DE", **DE_paras)
    control.QFIM()
    f_load = open('f.csv', 'r')
    f_load = ''.join([i for i in f_load])
...
m = MeasurementOpt(mtype="projection", minput=[], savefile=True, method="DE", **DE_paras)
