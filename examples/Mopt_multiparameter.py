...
Measopt = MeasurementOpt(mtype='input', minput=["rotation", povm_basis, 4], savefile=False, method="AD", **AD_paras)
...
Measopt = MeasurementOpt(mtype='input', minput=["rotation", povm_basis, 4], savefile=False, method="PSO", **PSO_paras)
Measopt.dynamics(tspan, rho0, H0, dH, decay=decay)
Measopt.CFIM()

# measurement optimization algorithm: DE
DE_paras = {"popsize":10, "measurement0":[], "max_episode":100, "c":1.0, "cr":0.5, "seed":1234}
Measopt = MeasurementOpt(mtype='input', minput=["rotation", povm_basis, 4], savefile=False, method="DE", **DE_paras)
Measopt.dynamics(tspan, rho0, H0, dH, decay=decay)
Measopt.CFIM()
