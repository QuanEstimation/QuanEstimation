...
class CramerRao():
    ...
    def QFIM(self, rho, drho, dtype='SLD', rep='original', rho_type = 'density_matrix', exportLD=False):
        """
       ...
       rho_type:
        ...
        if rho_type == 'Bloch_vector':
            ...
            self.Lambda = suN_generator(dim)
            rho = np.identity(dim)/dim
            for i in range(len(r)):
                rho = rho + np.sqrt((dim-1)/(2*dim))*r[i]*self.Lambda[i]

            drho = [[] for i in range(0, para_num)]
            for para in range(para_num):
                drho[para] = np.zeros((dim))
                for j in range(len(r)):
                    drho[para] = drho[para] + np.sqrt((dim-1)/(2*dim))*dr[para][j]*self.Lambda[j]
        #=========================================================== 
        
        if dtype=='SLD':
            LD_tp = self.SLD(rho, drho, rep)
            for para_i in range(0, para_num):
                for para_j in range(para_i, para_num):
