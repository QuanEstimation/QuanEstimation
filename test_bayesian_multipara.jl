...
yspan = range(pi/2.0-0.1, stop=pi/2.0+0.1, length=10)
x1, x2 = xspan[1], xspan[end]
y1, y2 = yspan[1], yspan[end]
p = (1.0/(x2-x1))*(1.0/(y2-y1))*ones((length(xspan), length(yspan)))

rho_all = Matrix{Matrix{ComplexF64}}(undef, length(xspan), length(yspan))
drho_all = Matrix{Vector{Matrix{ComplexF64}}}(undef, length(xspan), length(yspan))
for i in 1:length(xspan)
    for j in 1:length(yspan)
        H0 = H0_res(xspan[i], yspan[j])
        dH = dH_res(xspan[i], yspan[j])
        rho, drho = expm(H0, dH, rho0, decay_opt, γ, [zeros(ComplexF64,2,2)], [zeros(length(tspan)-1)],tspan)
        rho_all[i,j] = rho[end]
        drho_all[i,j] = drho[end]
    end
...
M = [M1, M2]
p_out, x_out = Bayes([xspan, yspan], p, rho_all, M, res_input; save_file=false)
println(x_out)
L_out, x_out = MLE([xspan, yspan], rho_all, M, res_input; save_file=false)
println(x_out)
