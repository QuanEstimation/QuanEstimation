...
# # input the dynamics data
# dynamics = QuanEstimation.Lindblad(opt, tspan, H0, dH, ctrl=ctrl, decay=decay)   
# # objective function: CFI
# obj = QuanEstimation.CFIM_obj(M=M) 
# # run the comprehensive optimization problem
# QuanEstimation.run(opt, alg, obj, dynamics; savefile=false)
...
# opt = QuanEstimation.SCopt(ctrl=ctrl, ctrl_bound=[-0.2, 0.2])
...
opt = QuanEstimation.CMopt(ctrl=ctrl, M=C, ctrl_bound=[-0.2, 0.2])

# # comprehensive optimization algorithm: DE
# alg = QuanEstimation.DE(p_num=10, max_episode=100, c=1.0, cr=0.5, 
#                         seed=1234)
# # input the dynamics data
# dynamics = QuanEstimation.Lindblad(opt, tspan, rho0, H0, dH, Hc=Hc, 
#                                    decay=decay)   
# # objective function: CFI
# obj = QuanEstimation.CFIM_obj(M=M) 
# # run the comprehensive optimization problem
...
opt = QuanEstimation.SCMopt(ctrl=ctrl, M=C, ctrl_bound=[-0.2, 0.2])
