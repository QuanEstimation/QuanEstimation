...
function QFIM_TimeIndepend(K::AbstractVector, dK::AbstractVector{T}, psi0::AbstractVector, eps::Number) where T<:Number
    ρt, ∂ρt_∂x = sum([K*psi0*psi0'*K' for K in K]), sum([dK*psi0*psi0'*K' + K*psi0*psi0'*dK' for K in K])
...
function QFIM_TimeIndepend(K::AbstractVector, dK::AbstractVector{T}, ρ0::AbstractMatrix, eps::Number) where T<:Number
    ρt, ∂ρt_∂x = sum([K*ρ0*K' for K in K]), sum([dK*ρ0*K' + K*ρ0*dK' for K in K]) 
    QFIM_pure(ρt, ∂ρt_∂x, eps)
end

# QFIM for pure state with kraus rep.
function QFIM_TimeIndepend(K::AbstractVector, dK::AbstractVector, psi0::AbstractVector)
    ρt, ∂ρt_∂x = sum([K*psi0*psi0'*K' for K in K]), [sum([dK*psi0*psi0'*K' + K*psi0*psi0'*dK' for K in K]) for dK in dK]
    QFIM_pure(ρt, ∂ρt_∂x)
end
