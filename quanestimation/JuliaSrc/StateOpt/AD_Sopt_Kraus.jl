...
function gradient_QFI!(AD::TimeIndepend_Kraus{T}, epsilon) where {T <: Complex}
    δF = gradient(x->QFIM_TimeIndepend(AD.K, AD.dK[1], x, AD.eps), AD.psi)[1]
    AD.psi += epsilon*δF
    AD.psi = AD.psi/norm(AD.psi)
end

function gradient_QFI_Adam!(AD::TimeIndepend_Kraus{T}, epsilon, mt, vt, beta1, beta2, eps) where {T <: Complex}
    δF = gradient(x->QFIM_TimeIndepend(AD.K, AD.dK[1], x, AD.eps), AD.psi)[1]
    StateOpt_Adam!(AD, δF, epsilon, mt, vt, beta1, beta2, eps) 
    AD.psi = AD.psi/norm(AD.psi)
end

function gradient_QFIM!(AD::TimeIndepend_Kraus{T}, epsilon) where {T <: Complex}
    δF = gradient(x->1/(AD.W*pinv(QFIM_TimeIndepend(AD.K, AD.dK, x, AD.eps), rtol=AD.eps) |> tr |>real), AD.psi) |>sum
    AD.psi += epsilon*δF
    AD.psi = AD.psi/norm(AD.psi)
end

function gradient_QFIM_Adam!(AD::TimeIndepend_Kraus{T}, epsilon, mt, vt, beta1, beta2, eps) where {T <: Complex}
    δF = gradient(x->1/(AD.W*pinv(QFIM_TimeIndepend(AD.K, AD.dK, x, AD.eps), rtol=AD.eps) |> tr |>real), AD.psi) |>sum
    StateOpt_Adam!(AD, δF, epsilon, mt, vt, beta1, beta2, eps) 
    AD.psi = AD.psi/norm(AD.psi)
end

function gradient_CFI!(AD::TimeIndepend_Kraus{T}, M, epsilon) where {T <: Complex}
    δI = gradient(x->CFIM_TimeIndepend(M, AD.K, AD.dK[1], x, AD.eps), AD.psi)[1]
    AD.psi += epsilon*δI
    AD.psi = AD.psi/norm(AD.psi)
end

function gradient_CFI_Adam!(AD::TimeIndepend_Kraus{T}, M, epsilon, mt, vt, beta1, beta2, eps) where {T <: Complex}
    δI = gradient(x->CFIM_TimeIndepend(M, AD.K, AD.dK[1], x, AD.eps), AD.psi)[1]
    StateOpt_Adam!(AD, δI, epsilon, mt, vt, beta1, beta2, AD.eps) 
    AD.psi = AD.psi/norm(AD.psi)
end

function gradient_CFIM!(AD::TimeIndepend_Kraus{T}, M, epsilon) where {T <: Complex}
    δI = gradient(x->1/(AD.W*pinv(CFIM_TimeIndepend(M, AD.K, AD.K, x, AD.eps), rtol=AD.eps) |> tr |>real), AD.psi) |>sum
    AD.psi += epsilon*δI
    AD.psi = AD.psi/norm(AD.psi)
end

function gradient_CFIM_Adam!(AD::TimeIndepend_Kraus{T}, M, epsilon, mt, vt, beta1, beta2, eps) where {T <: Complex}
    δI = gradient(x->1/(AD.W*pinv(CFIM_TimeIndepend(M, AD.K, AD.dK, x, AD.eps), rtol=AD.eps) |> tr |>real), AD.psi) |>sum
    StateOpt_Adam!(AD, δI, epsilon, mt, vt, beta1, beta2, AD.eps) 
    AD.psi = AD.psi/norm(AD.psi)
end

function info_AD_noiseless_QFIM(M, AD::TimeIndepend_Kraus{T}, mt, vt, epsilon, beta1, beta2, max_episode, Adam, save_file, sym, str1, str2, str3) where {T <: Complex}
