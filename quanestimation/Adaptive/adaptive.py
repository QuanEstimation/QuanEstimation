...
class adaptive():
    ...
    def CFIM(self, M, W=[], save_file=False):
        ...
        if self.dynamic_type == "dynamics":
            ...
            adaptive_kraus(self.x, self.p, M, self.rho0, self.K, self.dK, W, self.max_episode, self.eps, save_file)
        else:
            raise ValueError("{!r} is not a valid value for type of dynamics, supported values are 'dynamics' and 'kraus'.".format(self.dynamic_type))
        ...
        if self.dynamic_type == "dynamics":
            if self.para_num == 1:
                ...
                for i in range(len(self.H)):
                    dynamics = Lindblad(self.tspan, self.rho0, self.H[i], self.dH[i], \
                                        decay=self.decay, Hc=self.Hc, ctrl=self.ctrl)
                ...
                for i in range(len(p_list)):
                    dynamics = Lindblad(self.tspan, self.rho0, self.H_list[i], self.dH_list[i], \
                                        decay=self.decay, Hc=self.Hc, ctrl=self.ctrl)
                    rho_tp, drho_tp = dynamics.expm()
                    rho, drho = rho_tp[-1], drho_tp[-1]
                    F_tp = QFIM(rho, drho)
            ...
            m.dynamics(self.tspan, self.rho0, H_res, dH_res, Hc=self.Hc, ctrl=self.ctrl, decay=self.decay)
...
def adaptive_dynamics(x, p, M, tspan, rho0, H, dH, decay, Hc, ctrl, W, max_episode, eps, save_file):
    ...
    if para_num == 1:
        ...
        for ei in range(max_episode):
            ...
            for hj in range(p_num):
                ...
                H_tp = H[x_idx]
                dH_tp = dH[x_idx]
                dynamics = Lindblad(tspan, rho0, H_tp, dH_tp, decay=decay, Hc=Hc, ctrl=ctrl)
                rho_tp, drho_tp = dynamics.expm()
                rho[hj] = rho_tp[-1]
        ...
        y, xout, pout = [], [], []
        for ei in range(max_episode):  
            rho = [np.zeros((dim, dim), dtype=np.complex128) for i in range(len(p_list))]
    ...
    if para_num == 1:
        ...
        for ei in range(max_episode):  
            ...
            for hj in range(len(p_list)):
                idx_list = [np.argmin(np.abs(x[i]-(x_list[hj][i]+u[i]))) for i in range(para_num)]
                x_idx = int(sum([idx_list[i]*np.prod(np.array(p_shape[(i+1):])) for i in range(para_num)]))
                rho[hj] = sum([np.dot(Ki, np.dot(rho0, Ki.conj().T)) for Ki in K_list[x_idx]])
            print("The tunable parameter are %s"%(u))
            res_exp = input("Please enter the experimental result:\n")
            res_exp = int(res_exp)
            pyx_list = np.zeros(len(p_list))
